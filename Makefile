...
release:
	@if [ "$(OS)" == "" ] && [ "$(ARCH)" == "" ] && [ "$(UNAME)" == "windows" ] ; then\
		echo "Build windows"; \
		go build -o builds/securosys-hsm.exe cmd/securosys-hsm/main.go; \
		cd builds; \
	...
	@if [ "$(OS)" == "" ] && [ "$(ARCH)" == "" ] && [ "$(UNAME)" != "windows" ] ; then\
		echo "Build $(UNAME)"; \
		go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
	...
	@if [ "$(OS)" == "windows" ]; then\
		...
		GOOS=${OS} GOARCH="${ARCH}" go build -o builds/securosys-hsm.exe cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm.exe > securosys-hsm_SHA256SUM; \
		zip -9 ${ARTIFACT_NAME}_${OS}_${ARCH}.zip securosys-hsm.exe securosys-hsm_SHA256SUM; \
		shasum -a 256 ${ARTIFACT_NAME}_${OS}_${ARCH}.zip >> ${ARTIFACT_NAME}_SHA256SUMS; \
		...
		GOOS=${OS} GOARCH="${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
		zip -9 ${ARTIFACT_NAME}_${OS}_${ARCH}.zip securosys-hsm securosys-hsm_SHA256SUM; \
		shasum -a 256 ${ARTIFACT_NAME}_${OS}_${ARCH}.zip >> ${ARTIFACT_NAME}_SHA256SUMS; \
...
release-all:
	...
	for ARCH in amd64 arm64; do\
		...
		GOOS=darwin GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
	...
	for ARCH in 386 amd64; do\
        echo "Build Windows in ARCH: $${ARCH}"; \
		GOOS=windows GOARCH="$${ARCH}" go build -o builds/securosys-hsm.exe cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm.exe > securosys-hsm_SHA256SUM; \
	...
	for ARCH in 386 amd64 arm arm64; do\
		...
		GOOS=linux GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
	...
	for ARCH in 386 amd64 arm; do\
		...
		GOOS=freebsd GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
	...
	for ARCH in 386 amd64 arm; do\
		...
		GOOS=netbsd GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
	...
	for ARCH in 386 amd64 arm; do\
		...
		GOOS=openbsd GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm > securosys-hsm_SHA256SUM; \
	...
	for ARCH in amd64; do\
		...
		GOOS=solaris GOARCH="$${ARCH}" go build -o builds/securosys-hsm cmd/securosys-hsm/main.go; \
		cd builds; \
		shasum -a 256 securosys-hsm >> securosys-hsm_SHA256SUM; \
...
build:
	go build -o vault/plugins/securosys-hsm cmd/securosys-hsm/main.go
