...
integration-tests:
	...
	cd integrationTests/tests && go test -count=1 -tags="unit integration" -v -timeout 60m 2>&1 ./... | ${GOPATH}/bin/go-junit-report -iocopy -out ${TEST_RESULT_PATH}integration_$(TEST)_junit_report.xml -set-exit-code
...
test:
	go install github.com/jstemmer/go-junit-report/v2@latest
	cd tests && go test -count=1 -tags="unit integration" -v -timeout 30m 2>&1 ./... | ${GOPATH}/bin/go-junit-report -iocopy -out ${TEST_RESULT_PATH}junit_report.xml -set-exit-code
